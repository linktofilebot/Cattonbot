...
import requests
import tempfile
import threading # বট এবং ফ্ল্যাস্ক একসাথে চালানোর জন্য
...
BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "") # রেন্ডারে আপনার বট টোকেন দিন
...
<div class="container">
    ...
    <div id="manageEpBox" class="sec-box">
        ...
        <input type="text" id="epSchInput" placeholder="🔍 Search series name..." onkeyup="filterEpSeries()" style="border:1px solid var(--main); margin-bottom:10px;">
    ...
    <div id="manageBox" class="sec-box">
        ...
        <input type="text" id="bulkSchInput" placeholder="🔍 Search content title..." onkeyup="filterBulkItems()" style="border:1px solid var(--main); margin-bottom:15px;">
        <form action="/bulk_delete" method="POST">
            <div id="bulkItemsWrap" style="max-height: 500px; overflow-y: auto; border: 1px solid #333; padding: 10px;">
                {% for m in movies %}
                <div class="bulk-item" style="padding:10px; border-bottom:1px solid #222; display:flex; align-items:center; gap:10px;">
                    ...
                    <span class="b-title" style="flex:1;">{{ m.title }} ({{ m.type }})</span>
...
<script>
    ...
    // সার্চ ফিল্টার - বাল্ক একশন
    function filterBulkItems() {
        let input = document.getElementById('bulkSchInput').value.toLowerCase();
        let items = document.querySelectorAll('.bulk-item');
        items.forEach(item => {
            let title = item.querySelector('.b-title').textContent.toLowerCase();
            item.style.display = title.includes(input) ? "flex" : "none";
        });
    }

    // সার্চ ফিল্টার - ইপিসোড সিরিজ ড্রপডাউন
    function filterEpSeries() {
        let input = document.getElementById('epSchInput').value.toLowerCase();
        let options = document.getElementById('mSeries').options;
        for (let i = 0; i < options.length; i++) {
            let text = options[i].text.toLowerCase();
            options[i].style.display = (i === 0 || text.includes(input)) ? "block" : "none";
        }
    }
...
# --- ৪. Flask রাউটস ও টেলিগ্রাম বট লজিক ---
...
def cmd_upload(message):
    bot.reply_to(message, "🎬 মুভি আপলোড শুরু...\nপ্রথমে মুভির নাম (Title) পাঠান।")
...
def bot_get_video(message):
    ...
    if user_data.get(cid, {}).get('step') == 'video':
        bot.reply_to(message, "📥 ফাইল পাওয়া গেছে! ক্লাউডে আপলোড হচ্ছে, দয়া করে অপেক্ষা করুন...")
        try:
            file_id = message.video.file_id if message.content_type == 'video' else message.document.file_id
            file_info = bot.get_file(file_id)
            # সরাসরি ক্লাউডিনারিতে আপলোড লিঙ্ক পাঠিয়ে দেওয়া (এটি বড় ফাইলের জন্য কার্যকর)
            file_url = f"https://api.telegram.org/file/bot{BOT_TOKEN}/{file_info.file_path}"
            up = cloudinary.uploader.upload_large(file_url, resource_type="video")
            
            movies_col.insert_one({
                "title": user_data[cid]['title'], "year": datetime.now().year, "poster": "https://via.placeholder.com/500x750", 
                "backdrop": "https://via.placeholder.com/1280x720", "type": "movie", "video_url": up['secure_url'], "likes": 0
            })
            bot.send_message(cid, f"✅ সফলভাবে আপলোড হয়েছে!\nনাম: {user_data[cid]['title']}\nইউআরএল: {up['secure_url']}")
        except Exception as e:
